...
# sources: kernel.proto, service.proto, types.proto
...
class OpenStatus(betterproto.Enum):
    """
    Information about how open the pool is to new delegators. This type might
    be extended in future versions of the API.
    """
...
class ContractVersion(betterproto.Enum):
    """
    Version of smart contract. This type might be extended in future versions
    of the API.
    """
...
class CredentialType(betterproto.Enum):
    """
    The type of a credential. This type might be extended in future versions of
    the API.
    """
...
class UpdateType(betterproto.Enum):
    """
    The type of chain update. This type might be extended in future versions of
    the API.
    """
...
class TransactionType(betterproto.Enum):
    """
    The type of transaction. This type might be extended in future versions of
    the API.
    """
    ...
    """Deprecated in protocol version 7. Introduced in protocol version 2."""
...
class ProtocolVersion(betterproto.Enum):
    """
    The different versions of the protocol. This type might be extended in
...
class CooldownCooldownStatus(betterproto.Enum):
    """
    The status of a cooldown. When stake is removed from a validator or
    delegator (from protocol version 7) it first enters the pre-pre-cooldown
    state. The next time the stake snaphot is taken (at the epoch transition
    before a payday) it enters the pre-cooldown state. At the subsequent
    payday, it enters the cooldown state. At the payday after the end of the
    cooldown period, the stake is finally released. This type might be extended
...
class NodeInfoBakerConsensusInfoPassiveCommitteeInfo(betterproto.Enum):
    """
    The committee information of a node configured with validator keys but
    somehow the node is _not_ part of the current validation committee. This
    type might be extended in future versions of the API.
    """

    NOT_IN_COMMITTEE = 0
    """
    The node is started with validator keys however it is currently not in the
    validation committee. The node is __not__ baking.
    """
    ...
    validator keys do not match the current keys on the validator account. The
...
class BakerInfo(betterproto.Message):
    ...
    """
    Identity of the validator. This is actually the account index of the
    account controlling the validator.
    """
...
class BakerPoolInfo(betterproto.Message):
    """
    Additional information about a validator pool. This information is added
    with the introduction of delegation.
    """
...
class AccountStakingInfo(betterproto.Message):
    """
    Information about the account stake, if the account is either a validator
    or a delegator.
    """
...
class AccountStakingInfoBaker(betterproto.Message):
    ...
    """
    If present, any pending change to the delegated stake. Starting from
    Concordium Protocol Version 7 this will never be present, due to changes
    ...
    """
    Present if the account is currently a validator, i.e., it is in the
    validator committee of the current epoch.
    """
...
class TokenInfo(betterproto.Message):
    ...
    token_id: "plt.TokenId" = betterproto.message_field(1)
    """The unique token id."""

    token_state: "plt.TokenState" = betterproto.message_field(2)
...
class AccountInfo(betterproto.Message):
    ...
    account index serves the role of the validator id, if the account is a
    validator. Hence it is exposed here as well.
    """
    ...
    """
    Present if the account is a validator or delegator. In that case it is the
    information about the validator or delegator.
    """
    ...
    """
    The available (unencrypted) balance of CCD's of the account (i.e. that can
    be transferred or used to pay for transactions). This is the balance minus
    the locked amount. The locked amount is the maximum of the amount in the
    ...
    tokens: List["AccountInfoToken"] = betterproto.message_field(13)
    """The protocol level tokens (PLT) held by the account."""


@dataclass(eq=False, repr=False)
class AccountInfoToken(betterproto.Message):
    """Protocol level token (PLT)."""

    token_id: "plt.TokenId" = betterproto.message_field(1)
    """The unique token id"""

    token_account_state: "plt.TokenAccountState" = betterproto.message_field(2)
...
@dataclass(eq=False, repr=False)
class TokenInfoRequest(betterproto.Message):
    """Request for token information."""

    block_hash: "BlockHashInput" = betterproto.message_field(1)
    """Block in which to query the token information."""

    token_id: "plt.TokenId" = betterproto.message_field(2)
...
class RejectReason(betterproto.Message):
    ...
    """
    A `ConfigureBaker` transaction is missing one or more arguments in order to
    add a validator.
    """
    ...
    """
    Block production reward commission is not in the valid range for a
    validator.
    """
    ...
    non_existent_token_id: "plt.TokenId" = betterproto.message_field(55, group="reason")
    """
    The provided identifier does not match a token currently on chain.
    Introduced in protocol version 9.
    """

    token_update_transaction_failed: "plt.TokenModuleRejectReason" = (
        betterproto.message_field(56, group="reason")
    )
    """
...
class ContractTraceElement(betterproto.Message):
    ...
    """
    A contract was upgraded. This trace element can occur starting from
    protocol version 5.
    """
...
class ContractTraceElementUpgraded(betterproto.Message):
    """
    A previously interrupted contract was resumed. This message can occur
    starting from protocol version 5.
    """
...
class BakerStakeUpdatedData(betterproto.Message):
    """
    The associated transaction type can no longer be created starting from
    protocol version 4. Hence, this message does not occur anymore. The
    functionality was replaced in protocol version 4 by the `BakerConfigured`
    transaction type.
    """
...
class BakerEvent(betterproto.Message):
    """
    Events that may result from the `ConfigureBaker` transaction which adds,
    modify, or removes a validator pool starting in protocol version 4. Before
    protocol version 4, distinct transaction types (`BakerAdded`,
    `BakerRemoved`, `BakerStakeUpdated`, `BakerRestakeEarningsUpdated`, and
    `BakerKeysUpdated`) existed which emitted validator related events instead.
    """

    baker_added: "BakerEventBakerAdded" = betterproto.message_field(1, group="event")
    """
    A validator was added. This event is always accompanied by
    `BakerEvent::BakerSetRestakeEarnings`, and starting from protocol version 4
    also `BakerEvent::BakerSetOpenStatus`, `BakerEvent::BakerSetMetadataURL`,
    `BakerEvent::BakerSetTransactionFeeCommission`,
    `BakerEvent::BakerSetBakingRewardCommission` and
    `BakerEvent::BakerSetFinalizationRewardCommission` events in the same
    transaction.
    """

    baker_removed: "BakerId" = betterproto.message_field(2, group="event")
    """
    A validator was removed by a transaction sent by the validator that
    decreased the validator's own stake to 0. When a validator is removed, it
    results in its delegators targeting the pool to be moved to the passive
    delegation. The behavior of validators being removed changed in protocol
    version 7 (see https://proposals.concordium.com/updates/P7.html for more
    details). If the cause of the validator removal is a transaction from an
    existing validator that switched its staking behavior to `delegation`, the
    `DelegationEvent::BakerRemoved` is emitted instead of this event.
    """
    ...
    """
    The validator's stake was decreased. The behavior of decreasing the stake
    of validators changed in protocol version 7 (see
    https://proposals.concordium.com/updates/P7.html for more details).
    """
    ...
    """
    An existing delegation was removed by a transaction sent from a delegator
    that switched its staking behavior to being a validator. This event is
    always accompanied by `BakerEvent::BakerAdded`,
    `BakerEvent::BakerSetRestakeEarnings`, and starting from protocol version 4
    also `BakerEvent::BakerSetOpenStatus`, `BakerEvent::BakerSetMetadataURL`,
    `BakerEvent::BakerSetTransactionFeeCommission`,
    `BakerEvent::BakerSetBakingRewardCommission` and
    `BakerEvent::BakerSetFinalizationRewardCommission` events in the same
    transaction. If the account is a delegator in the current payday, it will
    remain so until the next payday with respect to staking reward payouts,
    although the delegation record will be removed from the account
    immediately. If the cause of the delegation removal is a transaction sent
    by the delegator that decreased the delegator's stake to 0, the
    `DelegationEvent::DelegationRemoved` is emitted instead of this event. This
    event was introduced in protocol version version 7.
    """
    ...
    """
    The validator has been suspended by a transaction sent from the validator
    itself. If the validator is suspended by the protocol (e.g., due to
    inactivity), the `BlockSpecialEvent::ValidatorSuspended` event is emitted
    instead of this event. This event can occur starting from protocol version
    8.
    """
    ...
    """
    The validator has been resumed by a transaction sent from the validator
    itself. This event can occur starting from protocol version 8.
    """
...
class BakerEventBakerAdded(betterproto.Message):
    """
    A validator was added. This message/event is always accompanied by
    `BakerEvent::BakerSetRestakeEarnings`, and starting from protocol version 4
    also `BakerEvent::BakerSetOpenStatus`, `BakerEvent::BakerSetMetadataURL`,
    `BakerEvent::BakerSetTransactionFeeCommission`,
    `BakerEvent::BakerSetBakingRewardCommission` and
    ...
    """
    The amount the account staked to become a validator. This amount is locked.
    """
...
class BakerEventBakerSetOpenStatus(betterproto.Message):
    """
    Updated open status for a validator pool. This message can occur starting
    from protocol version 4.
    """
...
class BakerEventBakerSetMetadataUrl(betterproto.Message):
    """
    Updated metadata url for a validator pool. This message can occur starting
    from protocol version 4.
    """
...
class BakerEventBakerSetTransactionFeeCommission(betterproto.Message):
    """
    Updated transaction fee commission for a validator pool. This message can
    occur starting from protocol version 4.
    """
...
class BakerEventBakerSetBakingRewardCommission(betterproto.Message):
    """
    Updated block production reward commission for validator pool. This message
    can occur starting from protocol version 4.
    """
...
class BakerEventBakerSetFinalizationRewardCommission(betterproto.Message):
    """
    Updated finalization reward commission for validator pool. This message can
    occur starting from protocol version 4.
    """
...
class BakerEventDelegationRemoved(betterproto.Message):
    """
    Removed an existing delegator. An existing delegation was removed by a
    transaction sent from a delegator that switched its staking behavior to
    being a validator. If the account is a delegator in the current payday, it
    will remain so until the next payday with respect to staking reward
    payouts, although the delegation record will be removed from the account
    immediately. If the cause of the delegation removal is a transaction sent
    by the delegator that decreased the delegator's stake to 0, the
    `DelegationEvent::DelegationRemoved` is emitted instead of this
    event/message. This message can occur starting from protocol version 7.
    """
...
class BakerEventBakerSuspended(betterproto.Message):
    ...
    itself. If the validator is suspended by the protocol (e.g., due to
    inactivity), the `BlockSpecialEvent::ValidatorSuspended` event is emitted
    instead of this event. This message can occur starting from protocol
    version 8.
    """
...
class BakerEventBakerResumed(betterproto.Message):
    """
    A validator has been resumed by a transaction sent from the validator
    itself. This message can occur starting from protocol version 8.
    """
...
class DelegationEvent(betterproto.Message):
    """
    Events that may result from the `ConfigureDelegation` transaction which
    adds, modify, or removes a delegator starting in protocol version 4. There
    was no delegation feature before protocol version 4.
    """
    ...
    """
    The delegator's delegation target (either a validator pool or passive
    delegation) was updated by a transaction from the delegator.
    """

    delegation_added: "DelegatorId" = betterproto.message_field(5, group="event")
    """
    A delegator was added. This event is always accompanied by
    `DelegationEvent::DelegationSetDelegationTarget`,
    `DelegationEvent::DelegationSetRestakeEarnings `, and
    `DelegationEvent::DelegationStakeIncreased` events in the same transaction.
    """

    delegation_removed: "DelegatorId" = betterproto.message_field(6, group="event")
    """
    A delegator was removed by a transaction sent from the delegator that
    decreased the delegators's stake to 0. If the account is a delegator in the
    current payday, it will remain so until the next payday with respect to
    staking reward payouts, although the delegation record will be removed from
    the account immediately. If the cause of the delegation removal is a
    transaction sent from a delegator that switched its staking behavior to
    being a validator, the `BakerEvent::DelegationRemoved` is emitted instead
    of this event.
    """
    ...
    """
    An existing validator was removed by a transaction sent from an validator
    that switched its staking behavior to `delegation`. This event is always
    accompanied by `DelegationEvent::DelegationAdded`,
    `DelegationEvent::DelegationSetDelegationTarget`,
    `DelegationEvent::DelegationSetRestakeEarnings `, and
    `DelegationEvent::DelegationStakeIncreased` events in the same transaction.
    When a validator is removed, it results in its delegators targeting the
    pool to be moved to the passive delegation. The behavior of validators
    being removed changed in protocol version 7. (see
    https://proposals.concordium.com/updates/P7.html for more details). If the
    cause of the validator removal is a transaction that decreased the
    validator's own stake to 0, the `BakerEvent::BakerRemoved` is emitted
    instead of this event. This event can occur starting from protocol version
    7.
    """
...
class AccountTransactionEffects(betterproto.Message):
    ...
    """
    A validator was added. The associated transaction type can no longer be
    created starting from protocol version 4. Hence, this effect does not occur
    anymore. The functionality was replaced in protocol version 4 by the
    `BakerConfigured` transaction type.
    """

    baker_removed: "BakerId" = betterproto.message_field(7, group="effect")
    """
    A validator was removed.  The associated transaction type can no longer be
    created starting from protocol version 4. Hence, this effect does not occur
    anymore. The functionality was replaced in protocol version 4 by the
    `BakerConfigured` transaction type.
    """
    ...
    """
    A validator's stake was updated. The associated transaction type can no
    longer be created starting from protocol version 4. Hence, this effect does
    not occur anymore. The functionality was replaced in protocol version 4 by
    the `BakerConfigured` transaction type.
    """
    ...
    """
    A validator's restake earnings setting was updated. The associated
    transaction type can no longer be created starting from protocol version 4.
    Hence, this effect does not occur anymore. The functionality was replaced
    in protocol version 4 by the `BakerConfigured` transaction type.
    """

    baker_keys_updated: "BakerKeysEvent" = betterproto.message_field(10, group="effect")
    """
    A validator's keys were updated. The associated transaction type can no
    longer be created starting from protocol version 4. Hence, this effect does
    not occur anymore. The functionality was replaced in protocol version 4 by
    the `BakerConfigured` transaction type.
    """
    ...
    """
    An encrypted amount was transferred. The associated transaction type can no
    longer be created starting from protocol 7. Hence, this effect does not
    occur anymore.
    """
    ...
    The associated transaction type can no longer be created starting from
    protocol 7. Hence, this effect does not occur anymore.
    """
    ...
    """
    A validator was configured. The details of what happened are contained in a
    list of `BakerEvents`. The associated transaction type is available
    starting from protocol version 4 and replaces the existing transaction
    types and effects from earlier protocols (`BakerAdded`, `BakerRemoved`,
    `BakerStakeUpdated`, `BakerRestakeEarningsUpdated`, and
    `BakerKeysUpdated`).
    """
    ...
    list of DelegatorEvents. This transaction type is available starting from
    protocol version 4.
    """

    token_update_effect: "plt.TokenEffect" = betterproto.message_field(
        20, group="effect"
    )
...
class AccountTransactionEffectsAccountTransfer(betterproto.Message):
    ...
    """Memo. This field can occur starting from protocol version 2."""


@dataclass(eq=False, repr=False)
class AccountTransactionEffectsBakerStakeUpdated(betterproto.Message):
    """
    A validator updated its stake. This is the result of a successful
    `UpdateBakerStake` transaction. The associated transaction type can no
    longer be created starting from protocol version 4. Hence, this message
    does not occur anymore. The functionality was replaced in protocol version
    4 by the `BakerConfigured` transaction type.
    """
...
class AccountTransactionEffectsEncryptedAmountTransferred(betterproto.Message):
    ...
    EncryptedAmountTransfer transaction. The associated transaction type can no
    longer be created starting from protocol 7. Hence, this message does not
    occur anymore.
    """
...
class AccountTransactionEffectsTransferredWithSchedule(betterproto.Message):
    ...
    memo: Optional["Memo"] = betterproto.message_field(3, optional=True, group="_memo")
    """
    Optional memo. This field can occur starting from protocol version 2.
    """
...
class AccountTransactionEffectsBakerConfigured(betterproto.Message):
    ...
    the list of `BakerEvents`. The associated transaction type is available
    starting from protocol version 4 and replaces the existing transaction
    types and effects from earlier protocols (`BakerAdded`, `BakerRemoved`,
    `BakerStakeUpdated`, `BakerRestakeEarningsUpdated`, and
    `BakerKeysUpdated`).
...
class AccountTransactionEffectsDelegationConfigured(betterproto.Message):
    ...
    contained in the list of DelegationEvents. The associated transaction type
    is available starting from protocol version 4 and replaces the existing
    transaction types and effects from earlier protocols (`BakerAdded`,
    `BakerRemoved`, `BakerStakeUpdated`, `BakerRestakeEarningsUpdated`, and
    `BakerKeysUpdated`).
    """
...
class AuthorizationsV0(betterproto.Message):
    ...
    validator stake threshold, for V1 there are more.
    """
...
class AuthorizationsV1(betterproto.Message):
    ...
    """
    Access structure for updating the cooldown periods related to validation
    and delegation.
    """
    ...
    create_plt: Optional["AccessStructure"] = betterproto.message_field(
        4, optional=True, group="_create_plt"
    )
    """
    Access structure for creating a protocol level token. This is present from
    protocol version 9.
    """
...
class CapitalBound(betterproto.Message):
    """
    The capital bound is a chain parameter that is set to guarantee network
    decentralization by preventing  a single validator from gaining excessive
    power in the consensus protocol. The capital bound is required to be set to
    a value greater than 0 (capital_bound > 0). The value roughly describes the
    maximum proportion of the total stake in the protocol (from all validators
    including passive delegation)  to the stake of a validator that a validator
    can achieve where the total stake of the validator is considered effective
    (meaning the validator's total stake is used for caculating the lottery
    power or finalizer weight in the consensus). Once a validator passes this
    bound, some of the validator's total stake no longer contributes to the
    validator's effective stake. Delegators are prevented from increasing their
    delegation to the pool  (via sending a `ConfigureDelegation` transaction)
    if that would cause the pool to exceed the capital bound cap. See the
    comment at the `delegated_capital_cap` type for the exact formula.
    """
...
class LeverageFactor(betterproto.Message):
    """
    The leverage factor (leverage bound) is a chain parameter that is set to
    guarantee that each validator  has skin in the game with respect to its
    delegators by providing some of the CCD staked at the validator's pool from
    the validator's funds.   The leverage factor is required to be set to a
    value greater than or equal to 1 (1 <= leverage_factor). The leverage
    factor is the maximum proportion of total stake of a validator (including
    the validator's own stake and the delegated  stake to the validator) to the
    validator's own stake (excluding delegated stake to the validator) that a
    validator can achieve where the total stake of the validator is considered
    effective (meaning the validator's total stake is used for caculating the
    lottery power or finalizer weight in the consensus). Once a validator
    passes this bound, some of the validator's total stake no longer
    contributes to the validator's effective stake. Delegators are prevented
    from increasing their delegation to the pool  (via sending a
    `ConfigureDelegation` transaction) if that would cause the pool to exceed
    the leverage bound cap. See the comment at the `delegated_capital_cap` type
...
class PoolParametersCpv1(betterproto.Message):
    ...
    """
    The maximum leverage that a validator can have as a ratio of total stake to
    equity capital.
    """
...
class BakerStakeThreshold(betterproto.Message):
    """
    Minimum stake needed to become a validator. This only applies to protocol
    version 1-3.
    """
...
class UpdatePayload(betterproto.Message):
    ...
    create_plt_update: "plt.CreatePlt" = betterproto.message_field(24, group="payload")
    """Create a new protocol-level token."""


@dataclass(eq=False, repr=False)
class SponsorDetails(betterproto.Message):
    ...
    sponsor: "AccountAddress" = betterproto.message_field(2)
...
class AccountTransactionDetails(betterproto.Message):
    ...
    sponsor: Optional["SponsorDetails"] = betterproto.message_field(
        4, optional=True, group="_sponsor"
    )
...
class UpdateDetails(betterproto.Message):
    """
    Details of an successful update instruction. These are free, and we only
    ever get a response for them if the update is successfully enqueued, hence
    no failure cases.
    """
...
class BlockItemSummary(betterproto.Message):
    ...
    token_creation: "plt.TokenCreationDetails" = betterproto.message_field(
        7, group="details"
    )
...
class BlockInfo(betterproto.Message):
    ...
    """
    The validator id of the account producing this block. Not provided for a
    genesis block.
    """
...
class PoolCurrentPaydayInfo(betterproto.Message):
    ...
    """
    Whether the validator has contributed a finalization proof in the current
    reward period.
    """
    ...
    """
    The effective equity capital of the validator for the current reward
    period.
    """
...
class PoolInfoResponse(betterproto.Message):
    ...
    leverage and capital bounds. Absent if the pool is removed.
    leverage_bound_cap for pool p: Lₚ = λ * Cₚ - Cₚ = (λ - 1) * Cₚ
    capital_bound_cap for pool p: Bₚ = max (0, floor( (κ * (T - Dₚ) - Cₚ) / (1
    - K) )) delegated_capital_cap for pool p = min (Lₚ, Bₚ) Where κ is the
    `CapitalBound`. λ is the `LeverageFactor`. T is the total staked capital on
    the whole chain (including passive delegation). Dₚ is the delegated capital
    of pool p. Cₚ is the equity capital (staked by the pool owner excluding
    delegated stake to the pool) of pool p.
    """
    ...
    protocol version does not support validator suspension (meaning prior
    protocol version 8) or the pool is removed.
    """
...
class LeadershipElectionNonce(betterproto.Message):
    ...
    epoch to make sure that validators cannot predict too far in the future
    when they will win the right to bake blocks.
    """
...
class ElectionInfo(betterproto.Message):
    """
    Response type for GetElectionInfo. Contains information related to
    validator election for a perticular block. The `ElectionInfo` will be the
    same for all blocks in the same epoch.  Moreover, from protocol version 4,
    it is the same for all blocks in the same payday.
    """
    ...
    """
    Block production lottery election difficulty. Present only in protocol
    versions 1-5.
    """
...
class ElectionInfoBaker(betterproto.Message):
    ...
    """
    The lottery power of the validator, rounded to the nearest representable
    "double".
    """
...
class BlockSpecialEvent(betterproto.Message):
    ...
    This includes minting new CCD, rewarding different validators and
    delegators, etc.
    """
    ...
    """
    Rewards issued to each validator at the end of an epoch for producing
    blocks in the epoch, in proportion to the number of blocks they
    contributed. This only occurs in protocol versions 1 to 3. From protocol
    version 4, it is replaced by `PaydayPoolReward` and `PaydayAccountReward`
    messages.
    """

    mint: "BlockSpecialEventMint" = betterproto.message_field(2, group="event")
    """
    Minting of new CCDs. Starting from protocol version 4, this event occurs in
    each payday block. Before protocol version 4, this event occured every
    block (slot).
    """
    ...
    """
    Foundation rewards paid out at a payday block starting from protocol
    version 4.
    """
    ...
    """
    Reward payment to the given account at a payday block (from passive
    deleagtion or delegation to a validator or being a validator) starting from
    protocol version 4. When listed in a block summary, the delegated pool of
    the account is given by the last `PaydayPoolReward` outcome included before
    this outcome.
    """
    ...
    """
    Payment distributed to a pool or passive delegators at a payday block
    starting from protocol version 4. When listed in a block summary, the
    reward distribution to the delagtors of the given pool is given by the
    following `PaydayAccountReward` outcomes included after this outcome.
    """
    ...
    """
    The protocol suspends validators due to inactivity. If the validator is
    suspended  by a transaction sent from the validator itself, the
    `BakerEvent::BakerSuspended` event is emitted instead of this event. The
    event can occur starting from protocol version 8.
    """
    ...
    """
    The protocol primes validators for suspension due to inactivity. The event
    can occur starting from protocol version 8.
    """
...
class BlockSpecialEventMint(betterproto.Message):
    """
    Minting of new CCDs. Starting from protocol version 4, this event occurs
    only in each payday block. Before protocol version 4, this event occured
    every block (slot).
    """
...
class BlockSpecialEventBlockReward(betterproto.Message):
    """
    Disbursement of fees from a block between the GAS account, the validator,
    and the foundation. It should always be that: ```transaction_fees +
...
class BlockSpecialEventPaydayAccountReward(betterproto.Message):
    """
    Reward payment to the given account at a payday block starting from
    protocol version 4. When listed in a block summary, the delegated pool of
    the account is given by the last `PaydayPoolReward` outcome included before
    this outcome. For example: PaydayPoolReward to pool 1 PaydayAccountReward
    to account 5 PaydayAccountReward to account 6 PaydayAccountReward to
    account 1 (is the payout to the validator itself) PaydayPoolReward to
    `None` PaydayAccountReward to account 10 PaydayAccountReward to account 3
    Means 5, 6 are receiving rewards from delegating to validator 1 and 10, 3
    are receiving rewards from passive delegation.
    """
...
class BlockSpecialEventPaydayPoolReward(betterproto.Message):
    ...
    starting from protocol version 4. When listed in a block summary, the
    reward distribution to the delagtors of the given pool is given by the
    following `PaydayAccountReward` outcomes included after this outcome. For
    example: PaydayPoolReward to pool 1 PaydayAccountReward to account 5
    PaydayAccountReward to account 6 PaydayAccountReward to account 1 (is the
    payout to the validator itself) PaydayPoolReward to `None`
    PaydayAccountReward to account 10 PaydayAccountReward to account 3 Means 5,
    6 are receiving rewards from delegating to validator 1 and 10, 3 are
    receiving rewards from passive delegation.
    """
...
class BlockSpecialEventValidatorSuspended(betterproto.Message):
    """
    The id of a validator that got suspended due to too many missed rounds. The
    message can occur starting from protocol version 8.
    """
...
class BlockSpecialEventValidatorPrimedForSuspension(betterproto.Message):
    ...
    epoch due to too many missed rounds. The message can occur starting from
    protocol version 8.
    """
...
class PendingUpdate(betterproto.Message):
    ...
    """
    Updates validator stake threshold. Is only relevant prior to protocol
    version 4.
    """
...
class NextUpdateSequenceNumbers(betterproto.Message):
    ...
    protocol_level_tokens: "SequenceNumber" = betterproto.message_field(22)
    """
...
class NodeInfoBakerConsensusInfo(betterproto.Message):
    ...
    """
    The node is configured with validator keys and is member of the validation
    committee.
    """
    ...
    """
    The node is configured with validator keys and is member of the validator
    and finalization committees.
    """
...
class NodeInfoBakerConsensusInfoActiveBakerCommitteeInfo(betterproto.Message):
    """
    Tagging message type for a node that is configured with validator keys and
    active in the current validation committee
    """
...
class NodeInfoBakerConsensusInfoActiveFinalizerCommitteeInfo(betterproto.Message):
    """
    Tagging message type for a node that is configured with validator keys and
    active in the current finalizer committee (and also validation committee).
    """
...
class NodeInfoNode(betterproto.Message):
    ...
    """
    Consensus info for a node that is not configured with validator keys. The
    node is only processing blocks and relaying blocks and transactions and
...
class SendBlockItemRequest(betterproto.Message):
    ...
    by the sender. Instead, validators are rewarded by the protocol for
    including them.
    """
    ...
    account_transaction_v1: "AccountTransactionV1" = betterproto.message_field(
        4, group="block_item"
    )
    """
    ...
    raw_block_item: bytes = betterproto.bytes_field(5, group="block_item")
    """
    A block item which has already been serialized to the format expected by
    the node.
    """
...
@dataclass(eq=False, repr=False)
class AccountTransactionV1Signatures(betterproto.Message):
    """
    Account transaction signatures from the sender and optionally the sponsor
    of the transaction.
    """

    sender_signatures: "AccountTransactionSignature" = betterproto.message_field(1)
    """Signatures from the sender of the transaction."""

    sponsor_signatures: Optional["AccountTransactionSignature"] = (
        betterproto.message_field(2, optional=True, group="_sponsor_signatures")
    )
...
@dataclass(eq=False, repr=False)
class AccountTransactionHeaderV1(betterproto.Message):
    """
    Header v1 of an account transaction with support for sponsored
    transactions.
    """
    ...
    sponsor: Optional["AccountAddress"] = betterproto.message_field(
        6, optional=True, group="_sponsor"
    )
    """
...
class TransferWithMemoPayload(betterproto.Message):
    """
    Payload of a transfer between two accounts with a memo. This message can
    occur starting from protocol version 2.
    """
...
@dataclass(eq=False, repr=False)
class PreAccountTransactionV1(betterproto.Message):
    """
    An unsigned account transaction v1. This is used with the
    `GetTransactionSignHash` endpoint to obtain the message to sign.
    """

    header: "AccountTransactionHeaderV1" = betterproto.message_field(1)
    payload: "AccountTransactionPayload" = betterproto.message_field(2)
...
@dataclass(eq=False, repr=False)
class AccountTransactionV1(betterproto.Message):
    """
    Account transactions v1 are messages which are signed and paid for by the
    sender account or by the sponsor.
    """

    signatures: "AccountTransactionV1Signatures" = betterproto.message_field(1)
    """
    The signatures on the transaction by the source account and other relevant
    parties such as sponsors.
    """

    header: "AccountTransactionHeaderV1" = betterproto.message_field(2)
...
class BlockItem(betterproto.Message):
    ...
    raw_block_item: bytes = betterproto.bytes_field(6, group="block_item")
    """
    A block item which has already been serialized to the format expected by
    the node.
    """
...
class BakerRewardPeriodInfo(betterproto.Message):
    """
    Information about a particular validator with respect to the current reward
    period. The below values are historical value from the last payday block.
    """
    ...
    """
    The effective stake of the validator for the consensus protocol. The
    returned amount accounts for delegation, capital bounds and leverage
    bounds.
    """

    commission_rates: "CommissionRates" = betterproto.message_field(3)
    """
    The effective commission rate for the validator that applies for the reward
    period.
    """
...
class QuorumCertificate(betterproto.Message):
    ...
    identified by their validator id as this is stable across protocols and
    epochs.
    """
...
class FinalizerRound(betterproto.Message):
    ...
    (identified by their validator id) that signed off the round.
    """
    ...
    """
    The finalizers (identified by their validator id) that signed off the in
    'round'.
    """
...
class WinningBaker(betterproto.Message):
    """
    Details of which validator won the lottery in a given round in consensus
    version 1.
    """
    ...
    """
    True if the validator produced a block in this round on the finalized
    chain, and False otherwise.
    """
...
class BakersAndFinalizers(betterproto.Message):
    ...
    finalizers: List["BakerId"] = betterproto.message_field(2)
    """
    The IDs of the validators that are finalizers. The order determines the
    finalizer index.
    """
...
class EpochBakers(betterproto.Message):
    ...
    """
    The validators and finalizers for the previous epoch. If the current epoch
    is 0, then this is the same as the validators for the current epoch.
    """
    ...
    """
    The validators and finalizers for the current epoch. If this is absent, it
    should be treated as the same as the validators for the previous epoch.
    """
    ...
    """
    The validators and finalizers for the next epoch. If this is absent, it
    should be treated as the same as the validators for the current epoch.
    """
...
class ConsensusDetailedStatus(betterproto.Message):
    ...
    """
    Which validators the node has seen legally-signed blocks with live parents
    ...
    """
    The validators and finalizers for the previous, current and next epoch,
...
class QueriesStub(betterproto.ServiceStub):
    ...
    async def get_account_transaction_v1_sign_hash(
        self,
        pre_account_transaction_v1: "PreAccountTransactionV1",
        *,
        timeout: Optional[float] = None,
        deadline: Optional["Deadline"] = None,
        metadata: Optional["MetadataLike"] = None
...
class QueriesBase(ServiceBase):
    ...
    async def get_token_info(
        self, token_info_request: "TokenInfoRequest"
    ) -> "TokenInfo":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)
    ...
    async def get_account_transaction_v1_sign_hash(
        self, pre_account_transaction_v1: "PreAccountTransactionV1"
    ) -> "AccountTransactionSignHash":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)
    ...
    async def __rpc_get_token_info(
        self, stream: "grpclib.server.Stream[TokenInfoRequest, TokenInfo]"
    ) -> None:
        request = await stream.recv_message()
        response = await self.get_token_info(request)
        await stream.send_message(response)
    ...
    async def __rpc_get_token_list(
        self, stream: "grpclib.server.Stream[BlockHashInput, plt.TokenId]"
    ) -> None:
        request = await stream.recv_message()
        await self._call_rpc_handler_server_stream(
            self.get_token_list,
    ...
    async def __rpc_get_account_transaction_v1_sign_hash(
        self,
        stream: "grpclib.server.Stream[PreAccountTransactionV1, AccountTransactionSignHash]",
    ) -> None:
        request = await stream.recv_message()
        response = await self.get_account_transaction_v1_sign_hash(request)
        await stream.send_message(response)
    ...
    def __mapping__(self) -> Dict[str, grpclib.const.Handler]:
        return {
            ...
            "/concordium.v2.Queries/GetTokenInfo": grpclib.const.Handler(
                self.__rpc_get_token_info,
                grpclib.const.Cardinality.UNARY_UNARY,
                TokenInfoRequest,
                TokenInfo,
            ),
            ...
            "/concordium.v2.Queries/GetTokenList": grpclib.const.Handler(
                self.__rpc_get_token_list,
                grpclib.const.Cardinality.UNARY_STREAM,
                BlockHashInput,
                plt.TokenId,
            ),
            ...
            "/concordium.v2.Queries/GetAccountTransactionV1SignHash": grpclib.const.Handler(
                self.__rpc_get_account_transaction_v1_sign_hash,
                grpclib.const.Cardinality.UNARY_UNARY,
