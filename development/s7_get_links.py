...
def _():
    ...
    erc1155_v1_contracts = [x for group in erc_1155_v1_contracts_groups for x in group]
    return


@app.cell
def _(CCD_BlockItemSummary, Collections, mongodb, print):
    def print_receive_names_for(contracts: list[int]):
        for contract_index in contracts: 
            actions = set()
            imp_txs = [x["tx_hash"] for x in mongodb.mainnet[Collections.impacted_addresses].find({"impacted_address_canonical":f"<{contract_index},0>"}).sort({"block_height": -1})]
            txs = [CCD_BlockItemSummary(**x) for x in mongodb.mainnet[Collections.transactions].find({"_id": {"$in": imp_txs}}).sort({"block_info.height": -1})]
            results = {}
            dd_per_token = {}
            for tx in txs:
...
@app.cell
def _(print_receive_names_for):
    # print_receive_names_for(erc721_v2_contracts)
    print_receive_names_for([2])
    return
...
def _(
    ...
    mongodb,
    print,
    remove_fake_token_link,
    save_fake_token_link,
):
    def inventory_set_owner(contract_index: int):
        instance_info = mongodb.mainnet[Collections.instances].find_one({"_id":f"<{contract_index},0>"})
        if instance_info:
            if instance_info.get("v0"):
                if instance_info.get("v0").get("name") == "init_trader":
                    ...
                    all_links = list(mongodb.mainnet[Collections.tokens_links_v3].find({"token_holding.contract":f"<{contract_index},0>"}))
                    q = []
        ...
        for tx in txs:
            ...
            for effect in tx.account_transaction.effects.contract_update_issued.effects:
                if effect.updated:
                    if effect.updated.receive_name in ["inventory.safe_transfer_from", "inventory.transfer_from", "inventory.transfer", "inventory.create", "inventory.close"]:
                        ...
                        elif effect.updated.receive_name in [ "inventory.safe_transfer_from", "inventory.transfer_from"]:
                        ...
                        dd_per_token[parsed.custom_token_id].append({"block_height": tx.block_info.height, "action" : effect.updated.receive_name, "owner": address_field}) 
        ...
        for token, dd in dd_per_token.items():
            ...
            elif (action["action"] in ["inventory.transfer", "inventory.safe_transfer_from", "inventory.transfer_from"]) and (action["owner"] is None):
                remove_fake_token_link(contract_index, token, action["owner"])
            else:
                save_fake_token_link(contract_index, token, action["owner"])
        return results
    return (inventory_set_owner,)
...
@app.cell
def _(inventory_set_owner):
    inventory_set_owner(77)
    return
...
@app.cell
def _(erc721_v1_contracts, erc721_v2_contracts, inventory_set_owner, mo):
