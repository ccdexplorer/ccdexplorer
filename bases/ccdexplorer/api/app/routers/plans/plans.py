...
from ccdexplorer.api.app.models import User
from ccdexplorer.api.app.state_getters import get_user_details
...
async def plans_set_plan(
    ...
    if user:
        user.plan = plan
        if plan == "free":
            user.plan_end_date = dt.datetime.now().astimezone(dt.UTC) + dt.timedelta(days=365)
        else:
