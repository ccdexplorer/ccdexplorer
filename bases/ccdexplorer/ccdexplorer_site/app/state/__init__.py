...
def get_paydays_per_day(
    ...
    if (
        (
            dt.datetime.now().astimezone(dt.timezone.utc) - req.app.paydays_per_day_last_requested
        ).total_seconds()
        < 60
        ...
        result = list(MongoTypePayday(**x) for x in db_to_use[Collections.paydays].aggregate(pp))[0]

        req.app.paydays_last_blocks_validated = (
            result.height_for_last_block - result.height_for_first_block + 1
        )
        req.app.paydays_per_day_last_requested = dt.datetime.now().astimezone(dt.timezone.utc)
...
async def get_exchange_rates(
    ...
    if (
        (
            dt.datetime.now().astimezone(dt.timezone.utc) - req.app.exchange_rates_last_requested
        ).total_seconds()
        < 10
        ...
        req.app.exchange_rates_last_requested = dt.datetime.now().astimezone(dt.timezone.utc)
...
def get_exchange_rates_ccd_historical(
    ...
    if (
        ...
        req.app.exchange_rates_ccd_historical_last_requested = dt.datetime.now().astimezone(
            dt.timezone.utc
        )
...
def get_and_save_user_from_collection(req: Request):
    if (
        ...
        req.app.users_from_collection = {x["token"]: SiteUser(**x) for x in list(result)}
...
async def get_nodes(
    ...
    if (
        ...
        try:
            response = await req.app.httpx_client.get(f"{req.app.api_url}/v2/mainnet/misc/nodes")
            response.raise_for_status()
        ...
        try:
            response = await req.app.httpx_client.get(f"{req.app.api_url}/v2/testnet/misc/nodes")
            response.raise_for_status()
        ...
        nodes = {"mainnet": mainnet_nodes, "testnet": testnet_nodes}  # type: ignore
        return nodes
