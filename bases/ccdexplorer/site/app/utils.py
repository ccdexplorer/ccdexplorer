...
# from ccdexplorer.site.app.classes.dressingroom import MakeUp
...
def humanize_age(timestamp: dt.datetime, now: dt.datetime | None = None) -> str:
    ...
    # rd.years == 0 here
    months = rd.months if rd.months > 0 else 1  # safety: treat 0 months (>=28 days) as 1 month
...
def instance_link_v2(value: CCD_ContractAddress, user=None, tags=None, net: str = "mainnet"):
...
def datetime_delta_format_between_dates(date1: dt.datetime, date2: dt.datetime | None = None):
...
def datetime_delta_format_since_parse(value: str):
    ...
    if value_parsed:
        delta = dt.datetime.now(dt.timezone.utc) - value_parsed.astimezone(dt.timezone.utc)
        return verbose_timedelta(delta)
    else:
        return ""
...
def datetime_delta_format_until(source_value: dt.datetime | str):
    value: dt.datetime = (
        dateutil.parser.parse(source_value) if isinstance(source_value, str) else source_value
    )
        ...
        if canonical not in app.addresses_to_indexes[net]:
            ...
            if api_response.ok:
                app.addresses_to_indexes[net].update(
                    {canonical: api_response.return_value[canonical]}  # type: ignore
                )

                account_index = api_response.return_value[canonical] if api_response.ok else None  # type: ignore
        ...
        if account_index not in app.indexes_to_addresses[net]:
            ...
            if api_response.ok:
                app.indexes_to_addresses[net].update(
                    {account_index: api_response.return_value[account_index]}  # type: ignore
                )

                account_address = (
                    api_response.return_value[canonical] if api_response.ok else account_address  # type: ignore
                )
                canonical = account_address[:29]  # type: ignore
                return account_address, canonical, account_index
...
class EventType:
    def __init__(self, event, update, emit, logged_events: list | None = None):
        self.event = event
        self.update = update
        self.emit = emit
        self.logged_events: list[EventType] = logged_events  # type: ignore
...
def parse_account_or_contract(key, value, net, user, tags, app):
    ...
    if isinstance(value, dict):
        ...
        if sec_key == "Account":
            ...
            contract = CCD_ContractAddress.from_index(contract_id["index"], contract_id["subindex"])
...
def token_amount_using_decimals_rounded(
    value: int, decimals: int | None = None, rounded_decimals: int | None = None
):
    if not decimals:
    ...
    return (
        f"<span class='ccd'>{(int(value) * (math.pow(10, -decimals))):,.{rounded_decimals}f}</span>"
    )
...
async def process_event_for_makeup(req: ProcessEventRequest):
    ...
    if isinstance(req.user, dict):
        req.user = SiteUser(**req.user)

    result = req.logged_event_from_collection.recognized_event  # type: ignore
    event_info = req.logged_event_from_collection.event_info  # type: ignore
    event_type = req.logged_event_from_collection.event_info.event_type  # type: ignore
    standard = req.logged_event_from_collection.event_info.standard  # type: ignore
    if result:
        if standard == "CIS-2":
            ...
            if isinstance(
                result,
                depositCIS2TokensEvent | withdrawCIS2TokensEvent | transferCIS2TokensEvent,
            ):
                token_address_as_class = await get_token_info_from_collection(
                    req,
                    CCD_ContractAddress.from_str(result.cis2_token_contract_address),  # type: ignore
                )
            else:
            ...
            return EventType(
                f"{event_type}",
                req.app.templates.get_template("tx/logged_events/custom_event.html").render(
                    {
                        "result": result,
                        "request": req,
                        "net": req.net,
...
def pagination_calculator(req: PaginationRequest) -> PaginationResponse:
    if req.returned_rows:
        ...
        from_ = req.requested_page * int(req.limit) + 1  # type: ignore
        to_ = (req.requested_page + 1) * int(req.limit)  # type: ignore
        next_ = False if (req.returned_rows < req.limit) else True  # type: ignore
        start_ = False if (req.returned_rows < req.limit) and (req.requested_page == 0) else True  # type: ignore
        prev_ = False if (req.returned_rows < req.limit) and (req.requested_page == 0) else True  # type: ignore
        ...
        num_of_pages = int(math.ceil(req.total_txs / req.limit))  # type: ignore
        ...
        action_string = req.action_string
        word_ = (
            req.word.replace("-", " ") if req.total_txs == 1 else f"{req.word.replace('-', ' ')}s"
        )

        from_ = req.requested_page * int(req.limit) + 1  # type: ignore
        to_ = min(req.total_txs, (req.requested_page + 1) * int(req.limit))  # type: ignore
...
async def get_url_from_api(url: str, httpx_client: httpx.AsyncClient) -> APIResponseResult:
...
async def put_url_from_api(url: str, httpx_client: httpx.AsyncClient, json_put_content: list):
    api_response = APIResponseResult(status_code=-1, duration_in_sec=-1, ok=False)
    response = None
    now = dt.datetime.now().astimezone(dt.UTC)
    try:
...
def create_dict_for_tabulator_display_for_contracts(net, app, tags: dict, contract_dict: dict):
    ...
    return {
        ...
        "creator": (
            account_link(contract_dict.get("v1")["owner"], net, app=app, tags=tags)  # type: ignore
            if contract_dict.get("v1")
            else account_link(contract_dict.get("v0")["owner"], net, app=app, tags=tags)  # type: ignore
        ),
        "name": f"<span class='ccd'>{contract_dict.get('v1')['name'][5:] if contract_dict.get('v1') else contract_dict.get('v0')['name'][5:]}</span>",  # type: ignore
        "address_download": f"{contract_dict['_id']}",
        "source_module_download": f"{contract_dict.get('source_module', '')}",
        "creator_download": f"{contract_dict.get('v1')['owner'] if contract_dict.get('v1') else contract_dict.get('v0')['owner']}",  # type: ignore
        "name_download": f"{contract_dict.get('v1')['name'][5:] if contract_dict.get('v1') else contract_dict.get('v0')['name'][5:]}",  # type: ignore
