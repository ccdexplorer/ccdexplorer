...
async def get_ajax_chain_parameters_html_v2(
    ...
    html = request.app.templates.get_template("/block/block_chain_parameters.html").render(
        {
            "cp": chain_parameters,
            "user": user,
            "tags": tags,
...
async def get_ajax_special_events_html_v2(
    ...
    primed = [x for x in special_events if x["validator_primed_for_suspension"] is not None]
    suspended = [x for x in special_events if x["validator_suspended"] is not None]
    html = request.app.templates.get_template("/block/block_special_events.html").render(
        {
            "se": special_events,
            "user": user,
            "tags": tags,
...
async def get_ajax_payday_account_rewards_tabulator(
    ...
    if request.app.env["NET"] == "mainnet":
        ...
        for reward in account_rewards:  # type: ignore
...
async def get_ajax_payday_pool_rewards_tabulator(
    ...
    if request.app.env["NET"] == "mainnet":
        ...
        for reward in pool_rewards:  # type: ignore
