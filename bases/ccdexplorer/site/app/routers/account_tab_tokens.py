...
@router.get("/account/tokens-tab-content/{net}/{account_id}", response_class=HTMLResponse)
async def tokens_tab_content(
    ...
    return request.app.templates.get_template("account/account_tokens.html").render(
        {
            "net": net,
...
async def get_ajax_plt_tokens_(
    ...
    if not api_return_result:
        error = f"Request error getting PLT tokens for account at {account_id} on {net}."
        ...
        if len(result_rows) > 0:
            for row in result_rows:
                tb_made_up_rows.append(create_dict_for_tabulator_display_for_plt_token(net, row))
