...
def prepare_delegators_data(new_delegators, delegators, delegators_in_block_dict, request):
    """
    ...
    for account_id, value in new_delegators.items():
        rows.append(
            {
                "account_index": from_address_to_index(account_id[:29], "mainnet", request.app),
    ...
    for row in delegators:
        pending_block = (
            delegators_in_block_dict.get(row.get("account"), {}).get("pending_change", {}) or {}
        )
        messages = []
...
async def get_account_pool_delegators(
    ...
    delegators_in_dict = {CCD_DelegatorInfo(**x).account: x for x in delegators_in_block}
    ...
    delegators_in_block_dict = {CCD_DelegatorInfo(**x).account: x for x in delegators_in_block}
    ...
    for dele in delegators_with_ids:
        ...
        dele.update({"account": from_address_to_index(dele["account"][:29], net, request.app)})
