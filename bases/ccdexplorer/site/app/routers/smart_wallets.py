...
from ccdexplorer.site.app.classes.dressingroom import MakeUp, MakeUpRequest, RequestingRoute
...
async def get_public_key_events(
    ...
    for event in logged_events["logged_events_selected"]:
        ...
        token_address = f"{event['recognized_event']['cis2_token_contract_address']}-{token_id}"
        api_result = await get_url_from_api(
...
async def get_public_key_tokens(
    ...
    balances_all = tokens_list["fungible"] | tokens_list["non_fungible"] | tokens_list["unverified"]
    ...
    pagination = pagination_calculator(pagination_request)
    html = request.app.templates.get_template("smart_wallets/public_key_cis2.html").render(
        {
            "tags": tags,
            "net": net,
            "request": request,
...
@router.get("/{net}/smart-wallet/{index}/{subindex}/{public_key}", response_class=HTMLResponse)
