...
from ccdexplorer.site.app.state import get_httpx_client, get_labeled_accounts, get_user_detailsv2
...
@router.get("/account/validator-tab-content/{net}/{account_id}", response_class=HTMLResponse)
...
@router.post("/{net}/account/baker-performance/{validator_id}/{days}", response_class=Response)
...
@router.get("/{net}/account/{account_address}/aliases-in-use", response_class=HTMLResponse)
async def get_account_aliases_in_use(
    ...
    if len(aliases_in_use) > 0:  # type: ignore
        html = request.app.templates.get_template("account/account_aliases_in_use.html").render(
            {
...
@router.get("/{net}/account/{validator_id}/staking-rewards-object", response_class=HTMLResponse)
async def get_validator_pool_apy_rewards(
    ...
    if pool_apy_object:
        html = request.app.templates.get_template("account/account_delegator_rewards.html").render(
            {
...
async def get_account_validator_transactions(
    ...
    if len(tx_result_transactions) > 0:
        for transaction in tx_result_transactions:
            ...
            dct["first_event"] = (
                classified_tx.events_list[0].update if classified_tx.events_list else None
            )
...
async def get_validator_tally(
    ...
    tally = api_result.return_value if api_result.ok else {"data": [], "total_row_count": 0}
    if not tally:
