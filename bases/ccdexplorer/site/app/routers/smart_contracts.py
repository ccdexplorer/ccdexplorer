...
from ccdexplorer.domain.mongo import MongoTypeTokensTag, MongoTypeInstance, MongoTypeModule
from ccdexplorer.site_user import SiteUser

# from ccdexplorer.GRPCClient import GRPCClient
...
async def ajax_track_item_id(
    ...
    html = request.app.templates.get_template("smart_contracts/tnt_logged_events.html").render(
        {
...
async def smart_contract_page(
    ...
    if api_result.ok:
        ...
        tag_information = MongoTypeTokensTag(**api_result.return_value) if api_result.ok else None
        api_result = await get_url_from_api(
...
def events_to_file(instance_address: str, all_logged_events: list):
    ...
    df["recognized_event.initial_status"] = df["recognized_event.initial_status"].fillna("")
