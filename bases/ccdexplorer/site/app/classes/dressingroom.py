...
from ccdexplorer.site.app.classes.Enums import *
from ccdexplorer.env import *
from ccdexplorer.site.app.utils import *
...
class MakeUp:
    ...
    async def get_schema_from_source(self, contract_address: CCD_ContractAddress):
        ...
        if result_from_cache:
            ...
            if (now - result_from_cache["timestamp"]).total_seconds() < 5:
                ...
                source_module_name = self.app.schema_cache[self.net][contract_address.to_str()][
                    "source_module_name"
                ]
    ...
    async def prepare_for_display(
        ...
        await self._classify_transaction(transaction, account_id, account_view=account_view)
    ...
    async def transform_for_tabulator(self, plt_page: bool = False):
        ...
        if self.additional_info:
            if self.additional_info.get("type") == "amount":
                type_additional_info = micro_ccd_no_decimals(self.additional_info["value"])
    ...
    async def set_possible_cns_domain_from_update(self, effect_updated: CCD_InstanceUpdatedEvent):
        self.cns_domain = CNSDomain()
        self.cns_domain.function_calls[effect_updated.receive_name] = effect_updated.parameter
        ...
        if effect_updated.receive_name == "BictoryCnsNft.getTokenExpiry":
            ...
            self.cns_domain.tokenId = self.cns_domain.decode_token_id_from(effect_updated.parameter)
    ...
    async def _classify_transaction(
        ...
        if t.type.type == TransactionClass.AccountTransaction.value:
            ...
            dct = {
                "start_1": "",
                "end_1": ("✅" if t.account_transaction.outcome == Outcome.success.value else "❌"),
            ...
            if t.account_transaction.outcome == Outcome.reject.value:
                ...
                if effects.module_deployed:
                    ...
                    if process_events:
                        ...
                        logged_events_source = api_result.return_value if api_result.ok else None
                    else:
                    ...
                    if self.makeup_request.requesting_route == RequestingRoute.transaction:
                        logged_events = []
                        try:
                        ...
                        for event_index, event in enumerate(effects.contract_initialized.events):
                            success = False
                            if logged_events_source:
                                logged_events, success = await self.try_logged_event_parsing(
                        ...
                        for effect_index, effect in enumerate(
                            ...
                            if effect.updated:
                                ...
                                if t.block_info.height < 6_000_000:
                                    await self.set_possible_cns_domain_from_update(effect.updated)
                            ...
                            if effect.interrupted:
                                ...
                                if process_events:
                                    for event_index, event in enumerate(effect.interrupted.events):
                                        success = False
                                        if logged_events_source:
                                            (
                                                logged_events,
                    ...
                    dct.update({"start_1": effects.account_transfer.amount, "memo": memo})
                    ...
                    self.classifier = TransactionClassifier.Baking
                    movement_str = (
                        "Increased" if effects.baker_stake_updated.update.increased else "Decreased"
                    )
                    new_event = EventType(
                    ...
                    new_event = EventType("Encrypted Amount Transferred", f"{memo}", None)
                    ...
                    self.classifier = TransactionClassifier.Transfer
                    self.amount = sum(
                        [int(x.amount) for x in effects.transferred_with_schedule.amount]
                    )
                    self.additional_info = {
                        "type": "amount",
                        "value": self.amount,
                    ...
                    new_event = EventType("Transferred with Schedule", memo if memo else None, None)
    ...
    async def try_logged_event_parsing(
        ...
        if result:
            success = True
            logged_event_from_collection = MongoTypeLoggedEventV2(**logged_event_from_collection)
    ...
    def _prepare(self, transaction: CCD_BlockItemSummary):
        if transaction.type.type == TransactionClass.AccountTransaction.value:
            self.csv = {
                ...
                "amount": (f"{(int(self.amount) / 1_000_000):.6f}" if self.amount else ""),
                "amount_currency": "CCD",
